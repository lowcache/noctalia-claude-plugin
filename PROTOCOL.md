...
downstream of it: the presence orb, tooltips, `cc.pulse` subscribers). The
...
- `<target>` is the plugin dispatch id: `<plugin-id>:<widget-entry>` —
  `lowcache/claude:pulse` for this install. Adapters must treat it as
...
## Downstream: the `cc.pulse` state mirror
...
Desktop widgets receive it via `noctalia.state.watch("cc.pulse", cb)`; bar
