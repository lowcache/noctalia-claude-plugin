# Gemini Guide — noctalia-claude-plugin
...
* **EVIDENCE OVER SPECULATION** - Do not guess APIs, option names, or attribute paths — 
  ...
  `Gemini` in the progression of this project should be the best available at the 
...
* **CONTINUITY** - Match existing project patterns; prefer minimal, idiomatic changes.
  ...
  global instructions found in Gemini home directory. Maintain best practices and 
...
* **TEAM COLLABORATION** - This is a collaborative and team based project between that of the user 
  `LowCache` and `Gemini`. 
* **PROOF OVER FABRICATION** - All assertions made by `Gemini` while working on this project should 
  be backed by evidence, source, or documentation and should not be created 
  as narrative by `Gemini` and provenance should be made available upon 
  request by the user `LowCache`. `Gemini` should use any skill, tool, or mcp-server that is available to them.
