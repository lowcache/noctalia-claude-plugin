...
### 2026-06-26 — remember tool durability gap: fsync required for crash safety (fix committed c5d03ac)
**Symptom:** Power-loss, panic, or system crash mid-write to a shared inbox note could result in: (a) a file flushed to disk but missing its directory entry (lost note), or (b) a successful `os.replace()` operation but the data file not flushed to stable storage, so the note is truncated/garbage on recovery.
**Root cause:** Atomic-publish pattern (`write temp, os.replace(temp, final)`) provides concurrency safety (readers see a complete file or nothing), but does not survive crashes: the data must be durably on disk before the rename, and the rename must be durable in the directory metadata after completion.
**Prevention:** Durability rule for atomic-publish: (1) fsync the file immediately before `os.replace()` to ensure data is durable, (2) fsync the containing directory immediately after `os.replace()` to ensure the rename is durable. In Python: `os.fsync(fd.fileno())` before replace, `os.fsync(os.open(dirname, os.O_RDONLY).fileno())` after. This is distinct from atomicity (which os.replace handles) and required for any multi-writer shared path that needs to survive crashes.
